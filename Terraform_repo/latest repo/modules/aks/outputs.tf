...
output "kubelet_identity_object_id" {
  value = length(azurerm_kubernetes_cluster.main.kubelet_identity) > 0 ? azurerm_kubernetes_cluster.main.kubelet_identity[0].object_id : ""
}
...
output "private_dns_zone_name" {
  value = join(".", slice(split(".", azurerm_kubernetes_cluster.main.private_fqdn), 1, length(split(".", azurerm_kubernetes_cluster.main.private_fqdn))))
}