module "aks-ukwest" {
  ...
  cluster_name                     = module.naming_ukwest.kubernetes_cluster.name
  network_plugin                   = var.aks_network_plugin
  vnet_subnet_id                   = module.network-ukwest[0].vnet_subnets[0]
  ...
  #agents_availability_zones        = var.aks_agents_availability_zones_ukwest
  ...
  net_profile_dns_service_ip        = var.aks_net_profile_dns_service_ip_ukwest
  net_profile_docker_bridge_cidr    = var.aks_net_profile_docker_bridge_cidr
  net_profile_service_cidr          = var.aks_net_profile_service_cidr_ukwest
  diagnostics = {
  ...
  depends_on = [module.network-ukwest, null_resource.uksouthprovisioningflag,azurerm_subnet_route_table_association.sn_rt_association_aks_firewall-ukwest]
}
