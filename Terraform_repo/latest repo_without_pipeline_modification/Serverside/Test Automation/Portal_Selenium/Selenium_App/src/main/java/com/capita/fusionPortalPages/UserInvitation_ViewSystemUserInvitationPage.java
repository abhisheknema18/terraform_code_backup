...
public class UserInvitation_ViewSystemUserInvitationPage extends Basepage {
	public UserInvitation_ViewSystemUserInvitationPage() {
	...
	@FindBy(xpath="//a[text()='View System User Invitations']")
	WebElement ViewSystemUserInvitationsSubMenu;
	...
	@FindBy(xpath="//span[text()='System User']/following-sibling::input")
	WebElement txtBoxFilterSystemUser;
	
	@FindBy(xpath="//span[text()='Invitation Name']")
	WebElement columnHeaderInvitationName;
	
	@FindBy(xpath="//span[text()='System User']")
	WebElement columnHeaderSystemUser;
	...
	@FindBy(xpath="//div[@id='inviteTokenTab:inviteTokenListForm:ShowSystemActiveTokens']")
	...
	public void navigateToViewUserInvitations() throws InterruptedException {
		if(adminTab.isDisplayed()) {
			...
			if(UserInvitationMenu.isDisplayed()) {
				...
				TimeUnit.SECONDS.sleep(3);
				ViewSystemUserInvitationsSubMenu.click();
				logger.info("click View System User Invitations sub menu");
				TimeUnit.SECONDS.sleep(3);
			}
			else {
	...
	public void verifyDefaultTActiveToggleState() {
		if (toggleBtnActive.getAttribute("aria-checked").equalsIgnoreCase("aria-checked")) {
	...
	public void verifyFilterByInvitationName(String textToFIlter) {
		String invitationNameFilterInput=textToFIlter;
	...
	public void verifyFilterBySystemUser(String textToFilter) {
		String systemUserFilterInput=textToFilter;
		txtBoxFilterSystemUser.sendKeys(systemUserFilterInput);
		wait.until(ExpectedConditions.invisibilityOf(loadingBarImage));
		...
		if(noRecordsFoundMsg.size()==0) {
			...
			for(WebElement temp:workGroupCodes) {
				Assert.assertTrue(temp.getText().toLowerCase().contains(systemUserFilterInput.toLowerCase()));
			}
			logger.info("Filter by Invitation name works fine");
		}
		else {
	...
	public void verifySortBySystemUser() throws InterruptedException {
		List<WebElement> systemUsersObtained=driver.findElements(By.xpath("//tbody[@id='inviteTokenTab:inviteTokenListForm:tokenList_data']/tr/td[2]"));
		List<String> systemUsersObtainedList=new ArrayList<>();
		for(WebElement temp:systemUsersObtained) {
			systemUsersObtainedList.add(temp.getText());
		}
		
		columnHeaderSystemUser.click();
		logger.info("Clicked Work Group Code Column");
		wait.until(ExpectedConditions.invisibilityOf(loadingBarImage));
		
		List<WebElement> systemUsersGrid=driver.findElements(By.xpath("//tbody[@id='inviteTokenTab:inviteTokenListForm:tokenList_data']/tr/td[2]"));
		List<String> systemUsersSortedList=new ArrayList<>();
		
		for(WebElement temp:systemUsersGrid) {
			systemUsersSortedList.add(temp.getText());
		}
		
		Collections.sort(systemUsersObtainedList);
		Collections.sort(systemUsersSortedList);
		Assert.assertTrue(systemUsersObtainedList.equals(systemUsersSortedList));
		logger.info("Sorted by Work Group Code Ascending");
		wait.until(ExpectedConditions.invisibilityOf(loadingBarImage));
		
		
		//Now Reverse Sorting by System User Name
		columnHeaderSystemUser.click();
		logger.info("Clicked Work Group Code Header");
		wait.until(ExpectedConditions.invisibilityOf(loadingBarImage));
		
		List<WebElement> systemUsersObtainedDescending=driver.findElements(By.xpath("//tbody[@id='inviteTokenTab:inviteTokenListForm:tokenList_data']/tr/td[2]"));
		List<String> systemUsersObtainedListDescending=new ArrayList<>();
		for(WebElement AppName:systemUsersObtainedDescending) {
			systemUsersObtainedListDescending.add(AppName.getText());
		}
		
		
		Collections.sort(systemUsersObtainedList, Collections.reverseOrder());
		Collections.sort(systemUsersObtainedListDescending, Collections.reverseOrder());
		
		Assert.assertTrue(systemUsersObtainedList.equals(systemUsersObtainedListDescending));
				...
				if(Iconcolor.equalsIgnoreCase("color: green;")&&countGreen==0) {
					...
					if(Iconcolor.equalsIgnoreCase("color: red;")&&countRed==0) {
						...
						if(Iconcolor.equalsIgnoreCase("color: grey;")&&countGrey==0) {
							...
							//If color is Blue, check for the invitation is Completed
							if(Iconcolor.equalsIgnoreCase("color: blue;")&&countBlue==0) {
								countBlue++;
								logger.info("Completed Invitations Verified for Blue icon verified successfully");
							}
	...
	public void verifyActiveInvitationExpanderFields() {
		...
		//Verify other columns display and not null values hence Assert False used for not null check
	...
	public void enterRevokeNotes(String revokeNotes) {
		RevokeNotes=revokeNotes;
	...
	public void verifyRevokedInvitationExpanderFields(String revokeNotes) {
