...
public class PageCodebase implements Serializable{
    ...
    public boolean containsWorkGroup(List<HPCWorkGroups> list,String workGroupCode) {
    	if(list != null) {
    		if(list.stream().filter(l->l.getWorkgroupCode().equals(workGroupCode)).findFirst().isPresent()) {
    			return true;
    		}
    	}
    	return false;
    ...
    public boolean containsUsers(List<HPCUsers> list,HPCUserId userCode) {
    	if(list != null) {
    		if(list.stream().filter(l->l.getId().equals(userCode)).findFirst().isPresent()) {
    			return true;
    		}
    	}
    	return false;
